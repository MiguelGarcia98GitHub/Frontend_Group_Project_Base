...
export const FilterInputTextData = () => {
    ...
    let newArrayOfIngredients: any = [];

    arrayOfIngredients.forEach((ingredient, i) => {
        newArrayOfIngredients.push(arrayOfIngredients[i].trim());
    });

    const newObject: any = {};
    newArrayOfIngredients.forEach((ingredient: any, i: any) => {
        newObject[`strIngredient${i + 1}`] = ingredient;
    });

    for (let i = 0; i < 20; i++) {
        if (!newObject.hasOwnProperty(`strIngredient${i}`)) {
            newObject[`strIngredient${i}`] = "";
        }
    }
